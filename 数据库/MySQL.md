...
这是大多数数据库系统默认的隔离级别【MySQL不是】。该事务隔离级别满足事务隔离性的简单定义：一个事务开始时，只能看见已经提交的事务所作的修改。也就是说，<font color='#e53e30'>一个事务从开始到提交之前，所做的任何修改对其他事务是不可见的。</font>因此这个隔离级别也叫 nonrepeatable read, 因为两次执行同样的查询，可能会得到不一样的结果。

> 3. repeatable read

该级别时MySQL默认事务的隔离级别。解决了脏读的问题。该级别保证了在同一个事务中多次读取同样记录的结果是一致的，但是repeatable read ==不能解决【幻读】的问题==。幻读就是指，当某个事务在读取某个范围内的记录 时，另一个事务又在该范围内插入了新的记录，当之前的事务再次读取某个范围内的记录时，会产生幻行。
